...
function taskThree() {
  ...
  if (party.getDay() == 0 || party.getDay() == 6){
...
function multiplier(valueOne, valueTwo) {
  if(typeof valueOne === "number" && typeof valueTwo === "number"){
    const summa = valueOne * valueTwo
    alert("Produkten är: " + summa)
  } else {
...
function taskFive() {
  for (let index = 0; index < fruits.length; index++) {
    if (fruits[index] === "apelsin" || fruits[index] === "päron") {
      trash.push(fruits[index])
    } else {
      eatable.push(fruits[index])
    }
  }
document.getElementById("answer-five").innerHTML = `<p><b>Ätligt:</b>${eatable}</p>
<p><b>Skräp:</b>${trash}</p>`;
}

/*Lånade en sida ur Daniels bok gällande <p>-element för jag fick verkligen inte till det med <br>*/
...
function taskSix() {
  persons.forEach(persons => {
    if(persons.age >=30){
      overThirty.push(persons.name)
    } else {
      underThirty.push(persons.name)
    }
    if (persons.married == true) {
      married.push(persons.name)
    } else {
      notMarried.push(persons.name)
    }
  });
  document.getElementById("answer-six").innerHTML = 
  `<p><b>Över 30:</b> ${overThirty}</p> 
  <p><b>Under 30:</b> ${underThirty}</p>
  <p><b>Gift:</b> ${married}</p> 
  <p><b>Ogift:</b> ${notMarried}</p>`;
}
...
const addMeSomeLineBreaks =
 ...
 function taskSeven() {
    const comma = addMeSomeLineBreaks.split(",").join(",<br>")
    document.getElementById("answer-seven").innerHTML = comma
  }
...
function taskNine() {
  let middag = new Date().getHours();
  const divKalas = document.querySelectorAll(".answer-container");
  divKalas.forEach(element => {
      middag >= 17 ? element.style.backgroundColor = "blue" : element.style.backgroundColor = "red";
  })
}
